...
router.post("/create-manager", authenticateToken, async (req, res) => {
  ...
  try {
    const { username, password, phoneNumber, name, email, role } = req.body;
    const userWithRoleOwner = await User.findOne({ _id: req.user._id, role: "Owner" });

    if (!userWithRoleOwner) {
      return res.status(403).json({ message: "Access denied. Only owners can create managers." });
    }
    ...
    const newUser = new User({
      ...
      role, // Assuming role is correctly sent from the frontend as a simple string
    });
    ...
    const mailOptions = {
      ...
      text: `Phone number for logging in: ${phoneNumber}, email for logging in: ${email} and password for logging in: ${password}`,
    };

    transporter.sendMail(mailOptions, async (error, info) => {
      if (error) {
        console.error("Error sending verification email:", error);
        // Consider marking the user as needing email verification in the database here
