...
router.post('/create-manager', authenticateToken, async (req, res) => {
    ...
    try {
        const { username, password, firstName, phoneNumber, lastName, email, userId } = req.body;
        const userWithRoleOwner = await User.findOne({ _id: req.user._id, role: 'Owner' });
        if (!userWithRoleOwner) {
            return res.status(403).json({ message: 'Access denied. Only owners can create properties.' });
        }
...
router.post('/update-manager/:id', authenticateToken, async (req, res) => {
    ...
    // Check if the user has the role of "owner" in the database
    try {
        const { username, password, firstName, phoneNumber, lastName, email, propertyId } = req.body;
        const userWithRoleOwner = await User.findOne({ _id: req.user._id, role: 'Owner' });
        if (!userWithRoleOwner) {
            return res.status(403).json({ message: 'Access denied. Only owners can create properties.' });
        }
...
router.delete('/deleteManager/:id', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (
            ...
            return res.status(403).json({ message: 'Access denied. You are not authorized to delete this property.' });
        }
        ...
        return res.status(500).json({ message: 'Failed to delete property by ID.' });
    }
});
...
router.get('/getManager/:id', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ message: 'User found successfully.', manager: user });
    } catch (error) {
        console.error('Error deleting property by ID:', error);
...
router.get('/getBookings', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const allBookings = bookings.map((b) => ({ ...b._doc, propertyName: properties.find((p) => p._id.toString() == b.propertyId.toString())?.name }));
...
router.get('/getAllBookings', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (!userHasAccess) {
            return res.status(403).json({ message: 'Access denied. Only owners and managers can view bookings.' });
        }
        const properties = await Property.find({ owner_user_id: req.user._id });
        ...
        const bookings = userHasAccess && req.user.role == "Manager" ? await Booking.find() : await await Booking.find();
...
router.post('/create-booking', authenticateToken, async (req, res) => {
    try {
        ...
        await room.save();
        return res.status(201).json({ message: 'Booking created successfully.', property: newBooking });
    } catch (error) {
...
router.get('/get-booking/:id', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ message: 'Booking found successfully.', booking: { ...booking._doc, user: user } });
    } catch (error) {
...
router.get('/assign-manager/:propertyId/:managerId', authenticateToken, async (req, res) => {
    // This route is protected, and only authenticated users with a valid token can access it.
    // You can access the user information from req.user.
    // Example: const userId = req.user.id;
    if (!req.user) {
    ...
    try {
        const userWithRoleOwner = await User.findOne({ _id: req.user._id, role: 'Owner' });
        if (!userWithRoleOwner) {
            return res.status(403).json({ message: 'Access denied. Only owners can assign.' });
        }
...
router.get('/assign-room/:roomId/:bookingId', authenticateToken, async (req, res) => {
    // This route is protected, and only authenticated users with a valid token can access it.
    // You can access the user information from req.user.
    // Example: const userId = req.user.id;
    if (!req.user) {
...
router.get('/getUnOcupiedRooms/:bookingId', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const booking = await Booking.findById(req.params.bookingId);
        const bookings = await Booking.find({ roomType: booking.roomType, propertyId: booking.propertyId, roomAssigned: undefined });
        const rooms = await Room.find({ roomType: booking.roomType, propertyId: booking.propertyId });
        for (let i = 0; i < rooms.length; i++) {
            for (let j = 0; j < bookings.length; j++) {
                let overlap = (new Date(booking.from).getTime() < new Date(bookings[j].from) && new Date(booking.to).getTime() < new Date(bookings[j].from)) || (new Date(booking.from).getTime() > new Date(bookings[j].to).getTime() && new Date(booking.to).getTime() > new Date(bookings[j].to));
                if (!overlap) {
                    filteredRooms.push(rooms[i])
                }
            }
        }
        return res.status(200).json({ message: 'Found rooms successfully.', rooms: filteredRooms });
    }
    catch (error) {
        console.error('Error registering user:', error);
